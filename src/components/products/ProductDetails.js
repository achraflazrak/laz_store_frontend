    ...
    <div className="card">
      ...
      <div className="card-body">
        <div className="card-text">
          {lang === "en"
            ? Parser().parse(product.desc_en)
            : Parser().parse(product.desc_fr)}
        </div>

        <div className="d-flex justify-content-between my-2">
          <span className="text-danger fw-bold">${product.selling_price}</span>
        ...
        <div className="d-flex justify-content-between my-2">
          ...
          {product.status ? (
            <span className="text-primary fw-bold">
              {lang === "en" ? "In Stock" : "Disponible"}
            </span>
          ) : (
            <span className="text-warning fw-bold">
              {lang === "en" ? "Out of Stock" : "Repture de Stock"}
            </span>
          )}
        </div>
        {calculateReviewAverage() > 0 && (
          <div className="d-flex align-items-center">
            <Rating
              ...
              size={24}
            />
            <span className="mx-2">({calculateReviewAverage()})</span>
          </div>
        )}
      </div>
      <hr />
