...
export const fetchProductsBySCategory = createAsyncThunk(
  "fetch/scategory/products",
  async ({ param, slug, page }) => {
    const response = await axios.get(
      `${BASE_URL}/${param}/${slug}/products?page=${page}`
    );
    return response.data;
  }
);
...
export const filterProducts = createAsyncThunk(
  ...
  async ({ params, page }) => {
    const response = await axios.post(
      `${BASE_URL}/filter/products?page=${page}`,
      {
        params,
      }
    );
...
export const searchProducts = createAsyncThunk(
  ...
  async ({ searchTerm, page }) => {
    const response = await axios.post(
      `${BASE_URL}/search/products?page=${page}`,
      {
        searchTerm,
      }
    );
...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    checkIfProductAlreadyReviewedByUser(state, action) {
      const exists = state.product.reviews.some(
        (review) =>
    ...
    removeProductReview(state, action) {
      const updatedProductReviews = state.product.reviews.filter(
...
export const {
  toggleShowSearchBox,
  checkIfProductAlreadyReviewedByUser,
  removeProductReview,
