...
export default function Home() {
  ...
  const { products, categories, all, loading, showSearchBox, error } = useSelector(
    (state) => state.product
  );
  ...
  const handleInputChange = (e) => {
    let exists = filterParams.params.find(
      (filter) => filter === e.target.value
    );
    if (exists) {
      const updatedFilters = filterParams.params.filter(
        (filter) => filter !== e.target.value
      );
      setFilterParams({
      ...
      {loading ? (
        ...
        <div className="row my-5 pb-5">
          ...
          <div className="col-md-8 card bg-white p-3">
            <ProductList
              products={products}
              fetchNextPrevProducts={fetchNextPrevProducts}
            />
          </div>
